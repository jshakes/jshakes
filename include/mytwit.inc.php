...
class myTwit{
	...
	function updateCache(){
		$uri = 'http://api.twitter.com/1/statuses/user_timeline.json?screen_name='.$this->user.'&count='.$this->postLimit.'&exclude_replies='.$this->excludeReplies;
	...
	function initMyTwit(){
		...
		for($x=0; $x < count($this->jsonArray) && $x < $this->postLimit; $x++){
			...
			$cur_ts = time();
			$output .= '<li'.$this->postClassAppend.'><span class="tcontent">&ldquo;'.$this->linkURLs($this->jsonArray[$x]['text']).
			'&rdquo;</span> <span class="twhen">by <a href="http://twitter.com/'.$this->jsonArray[$x]['user']['screen_name'].'"'.$this->targetAppend.'>'.$this->jsonArray[$x]['user']['screen_name'].'</a> '.
